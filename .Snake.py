...
class Snake:
    ...
    def turn_left(self):
        if self.x_velocity > 0:
            return
        self.x_velocity = self.speed*(-1)*self.block_size
        self.y_velocity = 0
        self.direction = "left"
    ...
    def turn_right(self):
        if self.x_velocity < 0:
            return
        self.x_velocity = self.speed*self.block_size
        self.y_velocity = 0
    ...
    def turn_down(self):
        if self.y_velocity < 0:
            return
        self.y_velocity = self.speed*self.block_size
        self.x_velocity = 0
        self.direction = "down"
    ...
    def turn_up(self):
        if self.y_velocity > 0:
            return
        self.y_velocity = self.speed*(-1)*self.block_size
        self.x_velocity = 0
        self.direction = "up"
...
class Game():
    ...
    def __init__(self, screen_width, screen_height, block_size, difficulty):
        # init pygame
        pygame.init()
        ...
        # instantiate snake
        self.snake = Snake(self.game_display, self.block_size,640,360)

        # instantiate fruit
        self.fruit = Fruit(self.screen_width, self.screen_height, self.block_size)
        self.fruit_extra = Fruit(self.screen_width, self.screen_height, self.block_size)
        if block_size == 10:
            self.head = pygame.image.load('head10.png')
            self.part = pygame.image.load('part10.png')
            self.apple = pygame.image.load('apple10.png')
        elif block_size == 20:
            self.head = pygame.image.load('head.png')
            self.part = pygame.image.load('part.png')
            self.apple = pygame.image.load('apple.png')
        elif block_size == 30:
            self.head = pygame.image.load('head30.png')
            self.part = pygame.image.load('part30.png')
            self.apple = pygame.image.load('apple30.png')
        elif block_size == 40:
            self.head = pygame.image.load('head40.png')
            self.part = pygame.image.load('part40.png')
            self.apple = pygame.image.load('apple40.png')
        elif block_size == 50:
            self.head = pygame.image.load('head50.png')
            self.part = pygame.image.load('part50.png')
            self.apple = pygame.image.load('apple50.png')
        self.apple_bonus = pygame.image.load('apple_bonus.png')
        # set window name
    ...
    def game_intro(self):
        ...
        while self.intro:
            ...
            self.game_display.blit(pygame.image.load('tło.png'), (0, 0))
    ...
    def main_loop(self):
        ...
        while self.running:
            ...
            if self.difficulty == 0:
                self.game_display.blit(pygame.image.load('tło3.png'), (0, 0))
                if self.check_collision(self.snake):
                    pygame.mixer.Sound.play(pygame.mixer.Sound("foghorn.wav"))
                    self.game_over = True
                    self.save_score()
            elif self.difficulty == 2:
                self.game_display.blit(pygame.image.load('tło5.png'), (0, 0))
                if self.check_collision(self.snake):
                    pygame.mixer.Sound.play(pygame.mixer.Sound("foghorn.wav"))
                    self.game_over = True
                    self.save_score()
                ...
                self.game_display.blit(pygame.image.load('tło4.png'), (0, 0))
            ...
            if self.check_fruit_collision(self.snake):
                ...
                self.foo1 = 0
                pygame.mixer.Sound.play(pygame.mixer.Sound("yipee.wav"))
            if self.check_bonusfruit_collision(self.snake):
                self.snake.add_segment()
                self.snake.add_segment()
                pygame.mixer.Sound.play(pygame.mixer.Sound("yipee.wav"))
            ...
            if len(self.snake.segments) % 5 == 0 and self.foo1 == 0:
                self.snake.speed += 0.1
            self.draw_score()
            self.draw_fruit(self.fruit)
            self.draw_snake(self.snake)
            if len(self.snake.segments) % 5 == 0:
                self.draw_bonus_fruit(self.fruit_extra)
    ...
    def check_collision_between(self, object1,object2):
    ...
    def check_collisions_special(self,collide):
        if collide.moving:
    ...
    def check_bonusfruit_collision(self, object):
        if object.segments[0].pos_y - 15 <= self.fruit_extra.pos_y <= object.segments[0].pos_y + 15 and \
                object.segments[0].pos_x - 15 <= self.fruit_extra.pos_x <= object.segments[0].pos_x + 15:
            return True
        return False
    ...
    def pause_game(self):
        ...
        while self.paused:
            ...
            self.clock.tick(30)
            pygame.display.update()
        pygame.mixer.Sound.play(pygame.mixer.Sound("start_message1.wav"))
    ...
    def game_controls(self):
        ...
        while self.gcont:
            ...
            self.game_display.blit(pygame.image.load('tło4.png'), (0, 0))
            self.button("Menu", 320 - 200, 300, 200, 80, self.red, self.light_red, action="menu1")
        ...
        while self.while_scores:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    quit()
            self.game_display.blit(pygame.image.load('tło4.png'), (0, 0))
            self.button("Menu", self.screen_width / 3 * 2 - 25, 600, 200, 80, self.red, self.light_red, action="menu")
            self.put_message("Najlepsze wyniki: ",-280)
            plik = open('scores.txt', 'r')
            y=-170
            i=1
            for line in plik:
        ...
        while self.descrpt:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    quit()
            self.game_display.blit(pygame.image.load('tło4.png'), (0, 0))
            self.button("Menu", self.screen_width /3 * 2 + 100, 300, 200, 80, self.red, self.light_red, action="menu1")
    ...
    def draw_bonus_fruit(self, fruit):
        self.game_display.blit(self.apple_bonus, [fruit.pos_x, fruit.pos_y, fruit.block_size, fruit.block_size])
...
class Game2(Game):
    def __init__(self,screen_width, screen_height, block_size,diff):
        ...
        self.computer = AI(self.game_display, self.block_size)
    def main_loop(self):
        pygame.mixer.music.load("snake_music2.mp3")
        pygame.mixer.music.play(-1)
        while self.running:
            ...
            if self.check_bonusfruit_collision(self.snake):
                ...
                self.snake.add_segment()
                pygame.mixer.Sound.play(pygame.mixer.Sound("yipee.wav"))
            if self.check_bonusfruit_collision(self.computer):
                self.computer.add_segment()
                self.computer.add_segment()
                pygame.mixer.Sound.play(pygame.mixer.Sound("yipee.wav"))
            ...
            # if len(self.snake.segments) % 5 == 0 and self.foo1 == 0:
            #     self.snake.speed += 0.1
            # if len(self.computer.segments) % 5 == 0 and self.foo2 == 0:
            #     self.computer.speed += 0.1
            self.draw_score()
            self.draw_score2()
            self.draw_fruit(self.fruit)
            if (len(self.snake.segments) % 5 == 0 or len(self.computer.segments) % 5 == 0) and self.computer.moving:
                self.draw_bonus_fruit(self.fruit_extra)
            self.draw_snake(self.snake)
...
class Game3(Game):
    def __init__(self, screen_width, screen_height, block_size, diff):
        super().__init__(screen_width, screen_height, block_size, diff)
        self.difficulty = diff
        self.snake2 = Snake2(self.game_display, self.block_size)
        self.foo1 = 1
        self.foo2 = 1
    ...
    def main_loop(self):
        ...
        while self.running:
            ...
            if self.check_bonusfruit_collision(self.snake):
                ...
                self.snake.add_segment()
                pygame.mixer.Sound.play(pygame.mixer.Sound("yipee.wav"))
            if self.check_bonusfruit_collision(self.snake2):
                self.snake2.add_segment()
                self.snake2.add_segment()
                pygame.mixer.Sound.play(pygame.mixer.Sound("yipee.wav"))
            ...
            self.draw_score2()
            self.draw_fruit(self.fruit)
            if (len(self.snake.segments) % 5 == 0 and not self.snake.moving)or (len(self.snake2.segments) % 5 == 0
                                                                                and not self.snake2.moving):
                self.draw_bonus_fruit(self.fruit_extra)
            if len(self.snake.segments) % 5 == 0 and self.foo1 ==0:
                self.snake.speed += 0.1
            if len(self.snake2.segments) % 5 == 0and self.foo2 ==0:
                self.snake2.speed += 0.1
            self.draw_snake(self.snake)
